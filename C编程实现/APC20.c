	...
	while(flag)
	{
       ...
       for(i=0;i<R;i++) 
       {
          ...
          for(k=0;k<R;k++) 
          {
              ...
              Rdsum=Rdsum+Rdis*Rdis; //sum of the changes
          }             
       }   
        ...
        for(j=0;j<R;j++) 
       {
          ...
          while(k1<R)
          {
              ...
              Adsum=Adsum+Adis*Adis;   //sum of the changes
              k1++;
          }   //while                   
       }  //for
	  ...
	  for(i=0;i<R;i++)
     {
        ...
        if((Rdaver<Threshold) && (Adaver <Threshold)||(times>MAXI))	
          flag=false;
...
////calculate the formula: max{a(i,k')+s(i,k')}
 ...
 //output the result to the file fp
 void outputRes(int in[],FILE *fp)
 {
 	int i,j,k=0;
	bool flag[R];
