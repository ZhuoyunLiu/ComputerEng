...
float data[R][Y];
char province[][]={"北京","天津","河北","山西","内蒙古","辽宁","吉林","黑龙江","上海","江苏","浙江","安徽","福建","江西","山东","河南","湖北","湖南","广东","广西","海南","重庆","四川","贵州","云南","西藏","陕西","甘肃","青海","宁夏","新疆"}; 
	...
	while(flag)
	{
       ...
       for(i=0;i<R;i++) 
       {
          ...
          for(k=0;k<R;k++) 
          {
              ...
              r[i][k]=(1-lam)*old+lam*r[i][k];  //formula2:r(new)=λ*r(new)+(1-λ)*r(old)
        ...
        for(j=0;j<R;j++) 
       {
          ...
          while(k1<R)
          {
              ...
              a[j][k1]=(1-lam)*old+lam*a[j][k1];  //formula:r(new)=λ*r(new)+(1-λ)*r(old)
